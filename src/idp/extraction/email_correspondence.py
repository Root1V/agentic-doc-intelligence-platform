"""Extractor for ``DocumentType.EMAIL_CORRESPONDENCE``. Same bounded
...
@register_extractor(DocumentType.EMAIL_CORRESPONDENCE)
class EmailCorrespondenceExtractor:
    schema_version = "1.0"
    ...
    def extract(self, parsed: ParsedDocument, settings: Settings, correction_note: str | None = None) -> ExtractionOutcome:
        try:
            result, trace = run_agentic_extraction(
                settings, parsed, EmailCorrespondenceSchema, DocumentType.EMAIL_CORRESPONDENCE, correction_note
            )
