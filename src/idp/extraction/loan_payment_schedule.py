"""Extractor for ``DocumentType.LOAN_PAYMENT_SCHEDULE``. Same bounded
...
@register_extractor(DocumentType.LOAN_PAYMENT_SCHEDULE)
class LoanPaymentScheduleExtractor:
    schema_version = "1.0"
    ...
    def extract(self, parsed: ParsedDocument, settings: Settings, correction_note: str | None = None) -> ExtractionOutcome:
        try:
            result, trace = run_agentic_extraction(
                settings, parsed, LoanPaymentScheduleSchema, DocumentType.LOAN_PAYMENT_SCHEDULE, correction_note
            )
