"""Extractor for ``DocumentType.FOREIGN_RESIDENT_ID``. Same bounded agentic
method as ``payslip.py``/``authorization_letter.py`` — see
``extraction/agentic/``."""
...
@register_extractor(DocumentType.FOREIGN_RESIDENT_ID)
class ForeignResidentIdExtractor:
    schema_version = "1.0"
    ...
    def extract(self, parsed: ParsedDocument, settings: Settings, correction_note: str | None = None) -> ExtractionOutcome:
        try:
            result, trace = run_agentic_extraction(
                settings, parsed, ForeignResidentIdSchema, DocumentType.FOREIGN_RESIDENT_ID, correction_note
            )
