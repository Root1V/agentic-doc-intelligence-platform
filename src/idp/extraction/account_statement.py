"""Extractor for ``DocumentType.ACCOUNT_STATEMENT``. Same bounded agentic
...
@register_extractor(DocumentType.ACCOUNT_STATEMENT)
class AccountStatementExtractor:
    schema_version = "1.0"
    ...
    def extract(self, parsed: ParsedDocument, settings: Settings, correction_note: str | None = None) -> ExtractionOutcome:
        try:
            result, trace = run_agentic_extraction(
                settings, parsed, AccountStatementSchema, DocumentType.ACCOUNT_STATEMENT, correction_note
            )
