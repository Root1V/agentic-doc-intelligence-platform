"""Extractor for ``DocumentType.CREDIT_SUMMARY``. Same bounded agentic
...
@register_extractor(DocumentType.CREDIT_SUMMARY)
class CreditSummaryExtractor:
    schema_version = "1.0"
    ...
    def extract(self, parsed: ParsedDocument, settings: Settings, correction_note: str | None = None) -> ExtractionOutcome:
        try:
            result, trace = run_agentic_extraction(
                settings, parsed, CreditSummarySchema, DocumentType.CREDIT_SUMMARY, correction_note
            )
